...
type FetchFn =
    Box<dyn Fn() -> Pin<Box<dyn Future<Output = Result<String, Errors>> + Send>> + Send + Sync>;
...
impl PromptRegistry {
    pub fn load(location: String) -> Result<Self, Errors> {
        let (scheme, path) = location.split_once("://").ok_or_else(|| {
            Errors::PromptRegistryError(format!(
                "Missing protocol in prompts location: {}",
                location
            ))
        })?;
        ...
        match scheme {
            ...
            _ => Err(Errors::PromptRegistryError(format!(
                "Unknown protocol '{}' in prompts location",
                scheme
            ))),
        }
    }
    ...
    fn load_from_filesystem(path: &str) -> Result<Self, Errors> {
        ...
        let registry = PromptRegistry { root };
        ...
        for entry in std::fs::read_dir(dir).map_err(|e| {
            ...
            if file_type.is_dir() {
                ...
                let fetch: FetchFn = Box::new(move || {
                    ...
                    Box::pin(
                        async move { tokio::fs::read_to_string(&file_path).await.map_err(io_err) },
                    )
                });
