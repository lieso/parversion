    ...
    for cluster in &basis_clusters {
        ...
        for network_lineage in &cluster.networks {
            ...
            if let Some(network) = network {
                ...
                println!("{}    transformations: {} count{}", MAGENTA, network.transformations.len(), RESET);

                for (txn_idx, transformation) in network.transformations.iter().enumerate() {
                    println!("{}    [Transformation {}]{}", MAGENTA, txn_idx + 1, RESET);
                    println!("{}      description: {}{}", MAGENTA, transformation.description, RESET);
                }
